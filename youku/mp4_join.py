...
def read_avc1(stream, size, left, type):
    ...
    'avcC', 'pasp'), 'if the sub atom is not avcC or pasp (actual %s), you should not cache raw body' % child.type
    left -= child.size
...
def merge_moov(moovs, mdats):
    ...
    stss = merge_stss((x.get('mdia', 'minf', 'stbl', 'stss').body[1] for x in trak0s),
                      (len(x.get('mdia', 'minf', 'stbl', 'stsz').body[3]) for x in trak0s))

    stsc0 = merge_stsc((x.get('mdia', 'minf', 'stbl', 'stsc').body[1] for x in trak0s),
                       (len(x.get('mdia', 'minf', 'stbl', 'stco').body[1]) for x in trak0s))
    stsc1 = merge_stsc((x.get('mdia', 'minf', 'stbl', 'stsc').body[1] for x in trak1s),
                       (len(x.get('mdia', 'minf', 'stbl', 'stco').body[1]) for x in trak1s))
